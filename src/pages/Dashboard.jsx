...
export default function Dashboard() {
  ...
  const cards = stats ? [
    { label: 'Total Registrations', value: stats.totalParticipants ?? 0,   icon: Users,       sub: 'All registered participants' },
    { label: 'Paid Participants',   value: stats.paidParticipants ?? 0,     icon: CreditCard,  sub: `₹${((stats.paidParticipants ?? 0) * 750).toLocaleString()} collected` },
        ...
        <div className="card" style={{ marginBottom: 20 }}>
          ...
          <div className="progress-bar">
            <div
              ...
              style={{ width: stats.totalParticipants > 0 ? `${(stats.paidParticipants / stats.totalParticipants) * 100}%` : '0%' }}
            />
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 8, fontSize: 10, color: 'var(--text-muted)' }}>
            ...
            <span>{stats.totalParticipants - stats.paidParticipants} pending</span>
          </div>
        </div>
      )}
        ...
        <div className="grid-2">
          <div className="card-sm">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {[
                ['Total Revenue',    `₹${((stats.paidParticipants ?? 0) * 750).toLocaleString()}`],
                ['Avg per Team',     stats.totalTeams > 0 ? `₹${Math.round((stats.paidParticipants * 750) / stats.totalTeams).toLocaleString()}` : '—'],
